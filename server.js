...
const io = const kyivTime = new Date().toLocaleString("uk-UA", { timeZone: "Europe/Kyiv" });
console.log(kyivTime);

app.use(express.static(path.join(__dirname)));

// Внутрішній стан системи в оперативній пам'яті сервера
let state = {
  orders: [], // Активні записи та замовлення в роботі
  settings: {
    // Таблиця розпізнавання нарядів 3х10 (код, опис, тривалість)
    naryads: [
    ...
    posts: [
      ...
      { id: 5, name: 'Пост 5', active: false, mechanic: '' },
      { id: 6, name: 'Пост 6', active: false, mechanic: '' },
      { id: 7, name: 'Пост 7', active: false, mechanic: '' }
    ]
  },
  warehouse: [],
  messages: []
...
io.on('connection', (socket) => {
  ...
  // Відправка актуального стану новому підключеному пристрою
  socket.emit('initialState', state);

  // Синхронізація оновлень замовлень
  socket.on('updateOrders', (orders) => {
    state.orders = orders;
    io.emit('ordersUpdated', state.orders);
  });

  // Синхронізація стану складу
  socket.on('updateWarehouse', (warehouse) => {
    state.warehouse = warehouse;
    io.emit('warehouseUpdated', state.warehouse);
  });

  // Синхронізація адміністративних налаштувань
...
server.listen(PORT, () => {
  console.log(`Cupra One UI Server running on http://localhost:${PORT}`);
});